...
void UartCore::set_baud_rate(int baud) {
    ...
    divisor = ((SYS_CLK_FREQ * 1000000) / (16 * baud))- 1;
...
int UartCore::rx_byte() {
    ...
    if (rx_fifo_empty()) {
        ...
        io_write(base_addr, RM_RD_DATA_REG, 0); //dummy writing
        return ((int) data);
    }
}
